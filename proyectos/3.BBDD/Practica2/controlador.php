...
if(isset($_REQUEST['accion'])){
    switch($_REQUEST['accion']){
        ...
        case 'verProyecto':
            $idProy = $_REQUEST['id'];
            foreach($_SESSION['proyectos'] as $proyecto){
                if(strcmp($proyecto['id'], $idProy) == 0){
                    $nombre = $proyecto['nombre'];
                    $fechIni = $proyecto['fechaIni'];
                    $fechaFinPrevista = $proyecto['fechaFinPrevista'];
                    $diasTranscurridos = $proyecto['diasTranscurridos'];
                    $porcentageCompletado = $proyecto['porcentageCompletado'];
                    $importancia = $proyecto['importancia'];

                }
            }
            header("Location: verProyecto.php?nombre=" . $nombre . "&fechaIni=" . $fechIni . "&fechaFinPrevista=" . 
        $fechaFinPrevista . "&diasTranscurridos=" . $diasTranscurridos . "&porcentageCompletado=" . $porcentageCompletado . 
        "&importancia=" . $importancia);
        break;
        case 'generarProyectos':
            header("Location: informeProyectos.php");
            break;
        default:
          break;    
    }
